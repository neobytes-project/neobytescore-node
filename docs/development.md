...
To develop neobytescore-node:

```bash
cd ~
git clone git@github.com:<yourusername>/neobytescore-node.git
...
We now will setup symlinks in `neobytescore-node` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
...
cd devnode
mkdir node_modules
touch neobytescore-node.json
touch package.json
```

Edit `neobytescore-node.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../neobytescore-node/bin/neobytescore-node start
```